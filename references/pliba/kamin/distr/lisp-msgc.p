...
var i: integer;
begin
   for i:=1 to MEMSIZE do new(memory[i]);
   for i:=1 to MEMSIZE-1 do memory[i]^.carval := memory[i+1];
   memory[MEMSIZE]^.carval := nil;
   freeSEXP := memory[1]
end; (* initMemory *)
...
begin
   ...
   then begin
           gc;
           if freeSEXP = nil (* garbage collection didn't help *)
           then begin
...
(* unmarkPhase - reset mark in ALL cons cells                    *)
   procedure unmarkPhase;
   var i: integer;
   begin
      for i := 1 to MEMSIZE do memory[i]^.mark := false
   end; (* unmarkPhase *)
...
(* markSExp - set mark in s and all cells accessible from it     *)
      procedure markSExp (s: SEXP);
      begin
         if not s^.mark
         then begin
                 s^.mark := true;
                 if s^.sxptype = LISTSXP
                 then begin
                         markSExp(s^.carval);
                         markSExp(s^.cdrval)
                      end
...
(* markValuelist - set mark in all cells accessible from vl      *)
      procedure markValuelist (vl: VALUELIST);
      begin
...
(* markEnv - set mark in all cells accessible from rho           *)
      procedure markEnv (rho: ENV);
      begin
         markValuelist(rho^.values)
      end; (* markEnv *)

(* markExp - set mark in all cells accessible from e             *)
      procedure markExp (e: EXP);
 
(* markExplist - set mark in all cells accessible from el        *)
         procedure markExplist (el: EXPLIST);
         begin
      ...
      begin (* markExp *)
         case e^.etype of
           VALEXP: markSExp(e^.sxp);
           VAREXP: ;
           APEXP: markExplist(e^.args)
         end
      end; (* markExp *)
   ...
   begin (* markPhase *)
      ...
      markSExp(trueValue);
      markEnv(globalEnv);
      if currentExp <> nil then markExp(currentExp);
      fd := fundefs;
      while fd <> nil do begin
         markExp(fd^.body);
         fd := fd^.nextfundef
         end;
      for i:=1 to stacktop-1 do markSExp (argStack[i])
   end; (* markPhase *)
...
(* scanPhase - reconstruct free list from cells not marked       *)
   ...
   begin
      ...
      for i := 1 to MEMSIZE do
        if not memory[i]^.mark (* memory[i] inaccessible *)
        then begin
                memory[i]^.carval := freeSEXP;
                freeSEXP := memory[i];
...
begin (* gc *)
   ...
   markPhase;
   scanPhase;
   writeln('collected ', collected:1,
         '(',round(100*collected/MEMSIZE):1,'%) records')
end; (* gc *)
